...
//Rare exceptions to indicate a bug in the _queens_memory code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_queens_memory = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_queens_memory]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.queens_memory.module"
